...
import videoiconorangeIcon from "@/assets/images/video-icon-orange.svg";
import savePrimaryIcon from "@/assets/images/save-primary.svg";
...
export function Component() {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await getCourseDetails({ courseId: courseUniqueId });
      ...
      setFormData({
        ...
        priceErr: false,
        priceErrMsg: "",
        category: courseData?.speciallization ?? "",
        categoryErr: false,
        categoryErrMsg: "",
        displayCount: "",
        displayCountErr: false,
      ...
      setDisplayEnrolment(courseData?.showDisplayEnrollmentCount);
      setCoverImageFileName(courseData?.image);
      ...
      courseData?.courseContent.map((item, index) => {
        ...
        if (item?.contentType === "Lesson" && item?.lessonType === "Text") {
          textLessons.push({
            lessonContent: item?.content ?? "",
            lessonContentErr: false,
            lessonContentErrMsg: "",
            lessonId: `${item?.id}` ?? uuidv4(),
            ...
            lessonUploadFileErr: false,
            lessonUploadFileErrMsg: "",
            courseContentUniqueId: item?.courseContentUniqueId,
          });
        }

        if (item?.contentType === "Lesson" && item?.lessonType === "Video") {
          videoLessons.push({
            lessonContent: item?.content ?? "",
            lessonContentErr: false,
            lessonContentErrMsg: "",
            ...
            lessonTypeErr: false,
            lessonTypeErrMsg: "",
            lessonUploadFile: item?.videoUrl ?? [],
            lessonUploadFileErr: false,
            lessonUploadFileErrMsg: "",
            uploadedLessonFiles:
              item?.videoUrl.map((item) => {
                return {
                  imgName: item,
                  cvFileLinks: item,
                  progress: 100,
        ...
        if (item?.contentType === "Lesson" && item?.lessonType === "Document") {
          documentLessons.push({
            ...
            lessonTypeErr: false,
            lessonTypeErrMsg: "",
            lessonUploadFile: item?.fileUrl ?? [],
            lessonUploadFileErr: false,
            lessonUploadFileErrMsg: "",
            uploadedLessonFiles:
              item?.fileUrl.map((item) => {
                return {
                  imgName: item,
                  cvFileLinks: item,
                  progress: 100,
      ...
      courseData?.courseContent.map((item, index) => {
        if (item?.contentType === "Quiz") {
          quizParent.push({
            ...
            quizTitleErr: false,
            quizTitleErrMsg: "",
            quizPassPercentage: item?.passingPercentage,
            courseContentUniqueId: item?.courseContentUniqueId,
          });

          quizChildList[index] = item?.quizes.map((quiz) => {
            if (quiz?.questionType === "Multiple Choices") {
              return {
                mcqId: `${quiz?.id}`,
                multipleQuestion: quiz?.question,
                multipleQuestionErr: false,
                multipleQuestionErrMsg: "",
                multipleAnswer: quiz?.correctAnswer,
                multipleAnswerErr: false,
                multipleAnswerErrMsg: "",
                allMultipleAnswerOptions: quiz?.options,
                quizUniqueId: quiz?.quizUniqueId,
                quizId: `${item?.id}`,
              };
            }

            if (quiz?.questionType === "Single Choice") {
              return {
                scqId: `${quiz?.id}`,
                singleQuestion: quiz?.question,
                singleQuestionErr: false,
                singleQuestionErrMsg: "",
                singleAnswer: quiz?.correctAnswer,
                singleAnswerErr: false,
                singleAnswerErrMsg: "",
                allSingleAnswerOptions: quiz?.options,
                quizUniqueId: quiz?.quizUniqueId,
                quizId: `${item?.id}`,
              };
            }

            if (quiz?.questionType === "True or False") {
              return {
                booleanId: `${quiz?.id}`,
                booleanQuestion: quiz?.question,
                booleanQuestionErr: false,
                booleanQuestionErrMsg: "",
                booleanAnswer: quiz?.correctAnswer[0],
                booleanAnswerErr: false,
                booleanAnswerErrMsg: "",
                quizUniqueId: quiz?.quizUniqueId,
                quizId: `${item?.id}`,
            ...
            if (quiz?.questionType === "Type Answer") {
              return {
                typeQId: `${quiz?.id}`,
                typeQuestion: quiz?.question,
                typeQuestionErr: false,
                typeQuestionErrMsg: "",
                typeAnswer: quiz?.answer,
                typeAnswerErr: false,
                typeAnswerErrMsg: "",
                quizUniqueId: quiz?.quizUniqueId,
                quizId: `${item?.id}`,
      ...
      const filteredQuizChildList = quizChildList.filter((item) => item);
      const mergedList = [];
      for (let i = 0; i < quizParent.length; i++) {
        mergedList.push(...filteredQuizChildList[i]);
      }
      setCourseContent([
        ...textLessons,
        ...videoLessons,
      ...
      setCourseQuizes(quizParent);
      const courseContent = [...courseData?.courseContent];
      const contentOrderList = courseContent.sort(
        (a, b) => indexOrder.indexOf(a?.order) - indexOrder.indexOf(b?.order)
      );

      const orderIdList = [];
      contentOrderList.map((item) => {
        orderIdList.push(`${item.id}`);
      });
      setCourseContentOrder(orderIdList);
    }
    fetchData();
  }, [courseUniqueId, getCourseDetails]);
  ...
  const handlemultiFileUploadCvLink = async (files) => {
    ...
    if (files.length > 0) {
      const documentMaxSize = 200 * 1024 * 1024; // 20 MB
      ...
      try {
        ...
        courseContent.map((item, index) => {
          if (item?.lessonType === "text") {
            ...
            textLessons.push({
              // id: item?.lessonId,
              courseUniqueId: courseDetails?.courseUniqueId ?? "",
              courseContentUniqueId: item?.courseContentUniqueId ?? "",
          ...
          if (item?.lessonType === "video") {
            ...
            videoLessons.push({
              ...
              order: courseContentOrder.indexOf(item?.lessonId),
              content: item?.lessonContent ?? "",
              videoUrl: item?.lessonUploadFile
                ? item.uploadedLessonFiles.map((item) => {
                    return item?.cvFileLinks;
                  })
                : [],
              fileUrl: [],
            });
        ...
        const finalQuizPayload = courseQuizes.map((quizItem, quizIndex) => ({
          id: quizItem?.id,
          courseUniqueId: courseDetails?.courseUniqueId ?? "",
          courseContentUniqueId: quizItem?.courseContentUniqueId ?? "",
          contentType: "Quiz",
          lessonType: null,
          ...
          quizes: courseContent
            .map((item, index) => {
              if (
                ...
                return {
                  quizUniqueId: item?.quizUniqueId ?? "",
  ...
  const getQuestionHeader = useMemo(() => {
    ...
    return isQuizQuestionTypeOpened
      ? `${isEditingCourseContent ? "Update" : "Add"} Question`
      : `${isEditingCourseContent ? "Update" : "Add"} Quiz`;
  }, [
    isChoosedQuestionType,
    isEditingCourseContent,
    isQuizQuestionTypeOpened,
  ...
  return (
    <AuthGuard>
      <Seo title="Edit Course" metaName="Metaname" metaTags={metaTags}>
              ...
              >
                <Grid item xs={12} sm={8}>
                  <Grid item container alignItems={"center"} columnGap={"20px"}>
                    <Link
                      to={`/instructor/course/course-details/${courseUniqueId}`}
                      className="backBtn"
                    >
                      <ArrowBackIcon />
                    </Link>
                ...
                <Grid item xs={12} sm={4} paddingTop={{ xs: 3, sm: 0 }}>
                  ...
                  >
                    <Grid item xs={12} sm="auto">
                      ...
                      >
                        <Link className="lightBtn">
                          <img src={savePrimaryIcon} />
                          Save Edit
                        </Link>
                      </div>
                    </Grid>
        ...
        <SuccessDialog
          ...
          title="Your updated course has been sent for another review"
        />
      </Seo>
    </AuthGuard>
  );
...
Component.displayName = "InstructorEditCourse";
